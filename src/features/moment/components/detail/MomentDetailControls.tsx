...
import { ArrowLeftOutlined, CommentOutlined, MenuOutlined } from "@ant-design/icons";
...
export default function MomentDetailControls({ item }: IProps) {
  ...
  const isMusic = useMemo(() => item.attributes?.type === "music", [item.attributes?.type]);
  const backgroundMusic = useMemo(() => item.attributes?.music ?? null, [item.attributes?.music]);
  ...
  return (
    <div className="relative w-full flex items-center justify-between px-3 z-10">
      ...
      <div className="relative flex items-center space-x-3 sm:space-x-5">
        ...
        <Twikoo id={item.id} show={showComment} setShow={setShowComment} />
        {!isMusic && backgroundMusic && <MomentAudioControl music={backgroundMusic} />}
      </div>
    </div>
  );
}
